...
function check_and_install {
    local packages=("kubelet" "kubeadm" "kubectl" "containerd.io" "docker-ce" "docker-ce-cli")
    ...
    # If all required packages are installed, print a message and return
    if $installed; then
        echo "All required packages are already installed."
        return
    fi

    # If not all required packages are installed, print a message and install the packages
    echo "Not all required packages are installed. Installing them now..."
...
function master_install {
    # call function check_and_install
    check_and_install
